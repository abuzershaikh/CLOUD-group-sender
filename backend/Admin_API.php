...
class Admin_API extends Controller
{
    ...
    public function save_campaign_status()
    {
        try {
            ...
            $db = \Config\Database::connect();
            $db->table($db->prefixTable('android_campaign_status'))->insert([
    ...
    public function list_campaign_status()
    {
        try {
            ...
            $db = \Config\Database::connect();
            $rows = $db->table($db->prefixTable('android_campaign_status'))
                ->where('team_id', $team->id)
                ->orderBy('created', 'DESC')
                ->get()
                ->getResultArray();
    ...
    public function get_campaign_status_detail()
    {
        try {
            ...
            $db = \Config\Database::connect();
            $row = $db->table($db->prefixTable('android_campaign_status'))
                ->where('team_id', $team->id)
                ->where('ids', $id)
                ->get()
                ->getRowArray();
    ...
    public function list_group_sender_status()
    {
        try {
            ...
            $campaignRows = $db->table($campaignTable)
                ->where('team_id', $team->id)
                ->whereIn('message_mode', ['group_text', 'group_media'])
                ->orderBy('created', 'DESC')
                ->get()
                ->getResultArray();
  ...
  private function check_api_key()
  {
    ...
    {
        $db = \Config\Database::connect();
        $campaign_table = $db->prefixTable('android_campaign_status');
