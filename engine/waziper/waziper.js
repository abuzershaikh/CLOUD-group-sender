...
const WAZIPER = {
	...
	makeWASocket: async function (instance_id) {
		...
		const WA = makeWASocket({
			...
			browser: ['Mac OS', 'Safari', '10.15.7'],             // Use Baileys recommended browser profile for QR auth
			...
			msgRetryCounterCache: new NodeCache(),          // Cache for message retry counters (required by Baileys)
			userDevicesCache: new NodeCache(),              // Cache for user device information (required by Baileys)
				...
				case "close":
					...
					if (lastDisconnect.error != undefined) {
						...
						if (DisconnectReason.loggedOut == statusCode) {
							...
							await Common.db_update("sp_accounts", [{ status: 0 }, { token: instance_id }]);
							await Common.db_delete("sp_groups", [{ ids: instance_id }]); // Delete old cached groups
			...
			if (messages.messages != undefined && messages.type == 'notify' || messages.messages != undefined && messages.type == 'append') {
				...
				if (messages.length > 0) {
					for (var i = 0; i < messages.length; i++) {
						...
						WAZIPER.saveRecentWAMessage(instance_id, message);
		...
		await WA.ev.on('groups.upsert', async (newGroups) => {
			WAZIPER.webhook(instance_id, { event: "groups.upsert", data: newGroups });
			try {
				if (newGroups && newGroups.length > 0) {
					var client = sessions[instance_id];
					if (client) {
						const existingDb = await Common.db_get('sp_groups', [{ ids: instance_id }]);
						let groupMap = new Map();
						if (existingDb && existingDb.data) {
							const parsedDb = JSON.parse(existingDb.data);
							if (Array.isArray(parsedDb)) parsedDb.forEach(g => { if (g && g.id) groupMap.set(g.id, g); });
						}
						
						const currentUserId = client.user ? client.user.id.replace(/:.*@/, '@') : null;
						
						newGroups.forEach(group => {
							if (!group || !group.id) return;
							const currentParticipant = Array.isArray(group.participants) ? group.participants.find((p) => p.id === currentUserId) : null;
							const userIsAdmin = currentParticipant?.admin === 'admin' || currentParticipant?.admin === 'superadmin';
							const userIsOwner = group.owner === currentUserId || group.subjectOwner === currentUserId;
						...
						client.groups = mergedGroups;
						
						const jsonString = JSON.stringify(mergedGroups);
						const now = Math.floor(Date.now() / 1000);
						if (existingDb) {
							await Common.db_update('sp_groups', [{ data: jsonString, changed: now }, { ids: instance_id }]);
						} else {
							const account = await Common.db_get('sp_accounts', [{ token: instance_id }]);
							await Common.db_insert('sp_groups', {
								ids: instance_id,
								team_id: account ? account.team_id : 0,
								name: `Instance ${instance_id} Groups`,
								data: jsonString,
								created: now,
								changed: now,
							});
				...
				console.log("[Groups Upsert Error]", e);
			}
		});
	...
	logout: async function (instance_id, res) {
		...
		if (sessions[instance_id]) {
			...
			// Session doesn't exist, but still clean up database
			console.log('Logout called for non-existent session:', instance_id, '- cleaning up database only');
			Common.db_delete("sp_whatsapp_sessions", [{ instance_id: instance_id }]);
			Common.db_update("sp_accounts", [{ status: 0 }, { token: instance_id }]);
	...
	fetch_groups_live: async function (client) {
	...
	refresh_groups_async: async function (instance_id) {
		var client = sessions[instance_id];
		if (!client || client._isRefreshingGroups) return;
		client._isRefreshingGroups = true;
		try {
			const groups = await WAZIPER.fetch_groups_live(client);
			if (groups && groups.length > 0) {
				const groupMap = new Map();

				// 1. Existing memory groups
				(client.groups || []).forEach(g => { if (g && g.id) groupMap.set(g.id, g); });

				// 2. Existing DB groups
				try {
					const existingDb = await Common.db_get('sp_groups', [{ ids: instance_id }]);
					if (existingDb && existingDb.data) {
						const parsedDb = JSON.parse(existingDb.data);
						if (Array.isArray(parsedDb)) {
							parsedDb.forEach(g => { if (g && g.id) groupMap.set(g.id, g); });
						}
					}
				} catch (_) {}

				// 3. Newly fetched groups
				groups.forEach(g => { if (g && g.id) groupMap.set(g.id, g); });

				const mergedGroups = Array.from(groupMap.values());
				client.groups = mergedGroups;

				const account = await Common.db_get('sp_accounts', [{ token: instance_id }]);
				const team_id = account ? account.team_id : 0;
				const jsonString = JSON.stringify(mergedGroups);
				const now = Math.floor(Date.now() / 1000);
				const existing = await Common.db_get('sp_groups', [{ ids: instance_id }]);
				if (existing) {
					await Common.db_update('sp_groups', [{ data: jsonString, changed: now }, { ids: instance_id }]);
				} else {
					await Common.db_insert('sp_groups', {
			...
			console.log(`[Groups Async Refresh Error] ${instance_id}:`, err.message);
		} finally {
			client._isRefreshingGroups = false;
		}
	},

	get_groups: async function (instance_id, res) {
		const health = await WAZIPER.check_session_health(instance_id, { attemptLoad: false, includeDbCheck: false });
		const isHealthy = health.healthy === true;
		const wsState = health.wsState || 'UNKNOWN';

		var client = sessions[instance_id];
		if (client == undefined) {
			return res.json({ 
				status: 'error', 
				message: 'Session is not connected or not ready',
				data: [],
		...
		// 1. In-Memory Cache (Instant 0ms) - with health check
		if (Array.isArray(client.groups) && client.groups.length > 0) {
			if (isHealthy) {
				// We still need to check DB to see if 10 days passed
				const dbGroup = await Common.db_get('sp_groups', [{ ids: instance_id }]);
				const now = Math.floor(Date.now() / 1000);
				const ten_days = 10 * 24 * 60 * 60;
				if (!dbGroup || !dbGroup.changed || (now - dbGroup.changed > ten_days)) {
					WAZIPER.refresh_groups_async(instance_id).catch(() => {});
				}
				...
				// ❌ FIX: Don't return cached groups if session unhealthy
				// This prevents showing stale groups from disconnected instances
				return res.json({ 
					status: 'error', 
					message: `Session not ready (${wsState}). Please reconnect to load groups.`,
					data: [],
					session_healthy: false,
					ws_state: wsState,
					reconnect_required: true
		...
		// 2. MySQL DB sp_groups Cache (Instant 2ms) - with health check
		try {
			const dbGroup = await Common.db_get('sp_groups', [{ ids: instance_id }]);
			if (dbGroup && dbGroup.data) {
				const parsed = JSON.parse(dbGroup.data);
				if (Array.isArray(parsed) && parsed.length > 0) {
					if (isHealthy) {
						client.groups = parsed;
						const now = Math.floor(Date.now() / 1000);
						const ten_days = 10 * 24 * 60 * 60;
						if (!dbGroup.changed || (now - dbGroup.changed > ten_days)) {
							WAZIPER.refresh_groups_async(instance_id).catch(() => {});
						}
						...
						// ❌ FIX: Don't return cached DB groups if session unhealthy
						return res.json({ 
							status: 'error', 
							message: `Session not ready (${wsState}). Please reconnect to load groups.`,
							data: [],
		...
		// 3. Fallback to Live Fetch if Cache is Empty
		if (!isHealthy) {
			return res.json({ 
				status: 'error', 
				message: `Session not ready (${wsState}). Please reconnect this instance.`,
				data: [],
				session_healthy: false,
				ws_state: wsState,
				reconnect_required: true
		...
		try {
			...
			console.log('[Groups] live fetch failed:', error.message);
			return res.json({ 
				status: 'error', 
				message: `Failed to load groups: ${error.message}`,
				data: [],
				session_healthy: false,
				ws_state: wsState
			});
	...
	check_session_health: async function (instance_id) {
		...
		// Check 1: Session exists in memory
		if (!session) {
			return { healthy: false, reason: 'Session does not exist in memory' };
		}

		// Check 2: User authenticated (critical for preventing "Cannot read properties of undefined" errors)
		if (!session.user || !session.user.id) {
			return { healthy: false, reason: 'Session not authenticated (missing user.id)' };
		}

		// Check 3: WebSocket connection open
		const wsState = session.ws?.readyState || session.ws?.socket?._readyState;
		const wsStateNames = ['CONNECTING', 'OPEN', 'CLOSING', 'CLOSED'];
		if (wsState !== 1) {
			return {
				healthy: false,
				reason: `WebSocket not open (state: ${wsState} - ${wsStateNames[wsState] || 'UNKNOWN'})`
			};
		}

		// Check 4: Database record exists
		const dbSession = await Common.db_get("sp_whatsapp_sessions", [{ instance_id: instance_id }]);
		if (!dbSession) {
			return { healthy: false, reason: 'Session not found in database' };
		}
  ...
  process_send_message: async function (chat_id, data, type, instance_id, phone_number, item, callback, type_media = '') {
      ...
      if (check_evo_acc && tp.includes(type_media)) {
        ...
        try {
          ...
          if (data.baileys_helper_content) {
            ...
            // ✅ FORWARD BADGE: Attach isForwarded contextInfo for group forward messages
            if (item && (item.message_mode === 'group_forward' || item.type === 'forward' || data.isForwarded)) {
              if (typeof data === 'object' && data !== null) {
                data.contextInfo = data.contextInfo || {};
                data.contextInfo.isForwarded = true;
                data.contextInfo.forwardingScore = 1;
                console.log('🏷️ Attached native WhatsApp Forward Tag contextInfo to message');
              }
            }
	...
	recentWAMessages: [],
	saveRecentWAMessage: function(instance_id, message) {
		try {
			if (!message || !message.key || !message.message) return;
			if (message.key.remoteJid === 'status@broadcast') return;

			const msgContent = message.message;
			const msgType = Object.keys(msgContent).find(k => k !== 'messageContextInfo' && k !== 'senderKeyDistributionMessage') || '';
			if (!msgType || msgType === 'protocolMessage') return;

			// Only capture incoming messages (not sent by us)
			...
			if (msgType === 'imageMessage') {
				...
				kind = 'media';
				mediaType = 'document';
				const fn = msgContent.documentMessage?.fileName || '';
				if (!text) text = fn ? `📄 Document (${fn})` : '📄 Document';
			...
			const existingIdx = WAZIPER.recentWAMessages.findIndex(m => m.id === message.key.id);
			if (existingIdx !== -1) {
				WAZIPER.recentWAMessages.splice(existingIdx, 1);
			}

			const senderJid = message.key.participant || message.key.remoteJid || '';
			const senderNum = senderJid.split('@')[0] || '';

			WAZIPER.recentWAMessages.unshift({
				id: message.key.id,
				instance_id: instance_id,
				kind: kind,
				text: text,
				...
				batch_name: `WA Message (${senderNum})`,
				source_message: {
					key: message.key,
			...
			if (WAZIPER.recentWAMessages.length > 100) {
				WAZIPER.recentWAMessages.pop();
			}
		} catch (e) {
			console.error('Error saving recent WAMessage:', e.message);
		}
	},

	limit: async function (item, type) {
		return true; // Limit check disabled: allow unlimited messaging
	},

	stats: async function (instance_id, type, item, status) {
		var time_now = Math.floor(new Date().getTime() / 1000);

		// Guard: initialize stats_history for team if not present
		if (!stats_history[item.team_id]) {
			stats_history[item.team_id] = {
				...
				wa_time_reset: time_now + 30 * 60 * 60 * 24,
				next_update: 0
			};
		}
